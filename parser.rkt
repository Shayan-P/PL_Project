...
(define-empty-tokens b (EOF sg-plus sg-minus sg-mul sg-div sg-pow sg-eq sg-eqeq sg-flash sg-lt sg-gt sg-semi sg-colon sg-comma sg-popen sg-pclose sg-sbopen sg-sbclose
                            kw-def kw-pass kw-cont kw-break kw-ret kw-global kw-if kw-else kw-in kw-for kw-and kw-or kw-not kw-true kw-false
                            kw-none kw-checked kw-int kw-float kw-bool kw-list))
...
(define simple-math-parser
  (parser
   ...
   (tokens a b)
   (grammar
    (program ((statements) (prog $1 #f))
                  ((kw-checked statements) (prog $2 #t)))
    ...
    (compound_stmt ((function_def) $1)
                   ((if_stmt) $1)
                   ((for_stmt) $1))
    (assignment ((IDENT sg-eq expression) (assign-stmt $1 (unknown-type) $3))
                ((IDENT sg-colon type-grammer sg-eq expression) (assign-stmt $1 $3 $5)))
    (return_stmt ((kw-ret) (return-novalue-stmt))
                 ((kw-ret expression) (return-value-stmt $2)))
    (global_stmt ((kw-global IDENT) (global-stmt $2)))
    (return-type ((sg-flash type-grammer) $2))
    (function_def ((kw-def IDENT sg-popen params sg-pclose sg-colon statements) (def-stmt $2 $4 (unknown-type) $7))
                  ((kw-def IDENT sg-popen sg-pclose sg-colon statements) (def-stmt $2 (list) (unknown-type) $6))
                  ((kw-def IDENT sg-popen params sg-pclose return-type sg-colon statements) (def-stmt $2 $4 $6 $8))
                  ((kw-def IDENT sg-popen sg-pclose return-type sg-colon statements) (def-stmt $2 (list) $5 $7)))
    (params ((param_with_default) (list $1))
            ((params sg-comma param_with_default) (append $1 (list $3))))
    (param_with_default ((IDENT sg-eq expression) (param-with-default $1 (unknown-type) $3))
                        ((IDENT sg-colon type-grammer sg-eq expression) (param-with-default $1 $3 $5)))
    ...
    (expressions ((expressions sg-comma expression) (append $1 (list $3)))
                 ((expression) (list $1)))
    (type-grammer 
          ((kw-int)   (primitive-type (int-type)))
          ((kw-float) (primitive-type (float-type)))
          ((kw-bool)  (primitive-type (bool-type)))
          ((kw-list)  (primitive-type (list-type)))
          ((kw-none)  (primitive-type (none-type)))))))
