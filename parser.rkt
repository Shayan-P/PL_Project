  ...
  (parser
   ...
   (grammar
    ...
    (compound_stmt ((function_def) $1)
                   ((if_stmt) $1)
                   ((for_stmt) $1))
    (assignment ((IDENT sg-eq expression) (assign-stmt $1 (empty-type) $3))
                ((IDENT sg-colon type-grammer sg-eq expression) (assign-stmt $1 (some-type $3) $5)))
    ...
    (function_def ((kw-def IDENT sg-popen params sg-pclose sg-colon statements) (def-stmt $2 $4 (empty-type) $7))
                  ((kw-def IDENT sg-popen sg-pclose sg-colon statements) (def-stmt $2 (list) (empty-type) $6))
                  ((kw-def IDENT sg-popen params sg-pclose return-type sg-colon statements) (def-stmt $2 $4 (some-type $6) $8))
                  ((kw-def IDENT sg-popen sg-pclose return-type sg-colon statements) (def-stmt $2 (list) (some-type $5) $7)))
    (params ((param_with_default) (list $1))
            ((params sg-comma param_with_default) (append $1 (list $3))))
    (param_with_default ((IDENT sg-eq expression) (param-with-default $1 (empty-type) $3))
                        ((IDENT sg-colon type-grammer sg-eq expression) (param-with-default $1 (some-type $3) $5)))
